			...
			<div>
				<PanelGroup>
					<Panel header="즐겨찾기 추가">
						이름 <input type="text" name="name" className="form-control" ref="nameInput" onChange={this.inputChange.bind(this)} />
						번호 <input type="text" name="phone" className="form-control" ref="phoneInput" onChange={this.inputChange.bind(this)} />
					</Panel>
				</PanelGroup>
				...
				<div style={styles}>
					{this.state.phoneDirectory.map((member, key) => {
						return (
							<Member key={key} name={member.name} phone={member.phone} />
						);
					})}
				</div>
			</div>
